...
def rmse(y_act,y_pred):
    ...
    return --> rmse agg
    """
    assert all(y_act[["SALES_DATE"]].dtypes.values == y_pred[["SALES_DATE"]].dtypes.values \
...
def rmse_sku(y_act,y_pred, asc_sort = False):
    ...
    df_join = y_act.merge(y_pred,on=["Encoded_SKU_ID","SALES_DATE"])
    # assert df_join.shape[1]==y_act.shape[1], "y_act and y_pred shapes do not match"
...
def plot_pred_sku(train,y_act,y_pred, sku_id, start_dt = "2022-07-01", ax1 = None):
    """
    y_act : pd df with Encoded_SKU_ID and SALES_DATE
    y_pred : pd df with Encoded_SKU_ID and SALES_DATE
    ...
    if ax1:
        ...
        ad_plot.title.set_text(
            f"Actual/Predicted sales of sku:{sku_id} from {df_join_subset.SALES_DATE.min():%Y-%m-%d} to {df_join_subset.SALES_DATE.max():%Y-%m-%d}")
    else:
        ad_plot = plt
        plt.title(
            f"Actual/Predicted sales of sku:{sku_id} from {df_join_subset.SALES_DATE.min():%Y-%m-%d} to {df_join_subset.SALES_DATE.max():%Y-%m-%d}")
    ad_plot.axvspan(y_pred_sku["SALES_DATE"].min(), y_pred_sku["SALES_DATE"].max(),\
