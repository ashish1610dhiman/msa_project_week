...
class stl_model():
    ...
    def fit_prophet(self,X_train1,X_test1):
        model = Prophet()
        ...
        #Train pred
        train_pred_p = model.predict(X_train_prophet)
        holiday_cols = [c for c in train_pred_p.columns if "Day" in c and "_" not in c]
        prophet_feats_train = train_pred_p[["trend", "holidays", "weekly", "yearly"] + holiday_cols]
        prophet_feats_train.index = X_train1.index
        y_train_prophet = train_pred_p["yhat"].rename({"yhat":"predicted"})
        y_train_prophet.index = X_train1.index
        #Test pred
        test_pred_p = model.predict(X_test_prophet)
        prophet_feats_test = test_pred_p[["trend", "holidays", "weekly", "yearly"] + holiday_cols]
        prophet_feats_test.index = X_test1.index
        y_test_prophet = test_pred_p["yhat"].rename({"yhat": "predicted"})
        y_test_prophet.index = X_test1.index
        return (model, prophet_feats_train, y_train_prophet, prophet_feats_test, y_test_prophet)
    ...
    def fit_models(self):
        ...
        model_name = "prophet"
        component="none"
        y_train1 = train_sku["DAILY_UNITS"]
        X_train1 = train_sku[self.pred_cols + ["DAILY_UNITS"]].astype(np.float64)
        X_test1 = validation_sku[self.pred_cols].astype(np.float64)
        prophet_model, prophet_feats_train, y_train_prophet,\
        prophet_feats_test, y_test_prophet = self.fit_prophet(X_train1,X_test1)
        train_pred[component + "_" + model_name] = pd.Series(y_train_prophet, index=X_train.index)
        y_pred_dict[component, model_name] = pd.Series(y_test_prophet, index=X_test.index)
        rmse_train = (np.mean((y_train1 - y_train_prophet) ** 2)) ** 0.5
        model_dict[(component, model_name)] = (prophet_model, rmse_train)
        print(f"{sku_id, component, model_name}, Train RMSE = {rmse_train:.4}")
        print ()

        #Attach feats from prophet to X_train and X_test
        X_train = X_train.merge(prophet_feats_train,left_index=True,right_index=True,how="left")
        X_test = X_test.merge(prophet_feats_test, left_index=True, right_index=True, how="left" )
    ...
    def get_result(self):
        ...
        y_pred_prophet = y_pred_dict["none","prophet"]
        ...
        y_pred = pd.DataFrame([y_pred_STL, y_pred_MSTL, y_pred_prophet]).T
        y_pred.columns = ["STL_prediction", "MSTL_prediction", "prophet_prediction"]
